...
private:
	...
	unsigned short	brN;
	unsigned short	brJ[768];
	unsigned short	brT1coarse[768];
	unsigned short	brT1fine[768];
	long long	brTime[768];
	long long	brTimeDelta[768];
	unsigned int	brChannelID[768];
	unsigned int	brChannelCount[768];
	float		brToT[768];
	float		brEnergy[768];
	unsigned short	brTacID[768];
	int		brXi[768];
	int		brYi[768];
	float		brX[768];
	float 		brY[768];
...
public:
        DataFileWriter(char *fName, double frequency, FILE_TYPE fileType, bool qdcMode, int hitLimitToWrite, int eventFractionToWrite, bool coincidence, int refChannel) {
		...
		if (fileType == FILE_ROOT){
			...
			hData->Branch("mh_j", brJ, "mh_j[768]/s");
			hData->Branch("t1coarse", brT1coarse, "t1coarse[768]/s");
			hData->Branch("t1fine", brT1fine, "t1fine[768]/s");
			hData->Branch("mh_j", brJ, "mh_j[768]/s");
			hData->Branch("tot", brToT, "tot[768]/F");
			hData->Branch("time", brTime, "time[768]/L");
			hData->Branch("timeDelta", brTimeDelta, "timeDelta[768]/L");
			hData->Branch("channelID", brChannelID, "channelID[768]/i");
			hData->Branch("channelCount", brChannelCount, "channelCount[768]/i");
			hData->Branch("energy", brEnergy, "energy[768]/F");
			hData->Branch("tacID", brTacID, "tacID[768]/s");
			hData->Branch("xi", brXi, "xi[768]/I");
			hData->Branch("yi", brYi, "yi[768]/I");
			hData->Branch("x", brX, "x[768]/I");
			hData->Branch("y", brY, "y[768]/I");
			hData->Branch("z", brZ, "z[768]/I");
			hData->Branch("tqT", brTQT, "tqT[768]/F");
                        hData->Branch("tqE", brTQE, "tqE[768]/F");
			...
			for(int m = 0; m < limit; m++) {
				...
				if (fileType == FILE_ROOT){
					...
					if( brChannelCount[h.raw->channelID] == 0 ) {
						...
						brT1fine[h.raw->channelID] = ProcessHit::remapADC(h.raw->t1fine);
						...
						brTQT[h.raw->channelID] = h.qT1;
						brTQE[h.raw->channelID] = h.qT2;
					}
                                        else {
					 	brChannelCount[h.raw->channelID] += 1;
					}
...
int main(int argc, char *argv[])
{
	...
	DataFileWriter *dataFileWriter = new DataFileWriter(outputFileName, reader->getFrequency(), fileType, reader->isQDC(), hitLimitToWrite, eventFractionToWrite, coincidence, refChannel);	

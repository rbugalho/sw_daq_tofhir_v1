...
//		if( decoder_log ) fprintf(decoder_log, "r = %d\n", r);
 		if (r != 5) continue;
 		
// 		if( decoder_log ) fprintf(decoder_log, "%3d %3d %3d %4d, '%s'\n", link, elink, event_number, 4*strlen(evt_hex), evt_hex );
		...
		if( decoder_log ) fprintf(decoder_log, "%3d %3d %3d '%12s' '%32s'", link, elink, event_number, tt_hex, evt_hex);
		
		if(evt_type != 0) {
			if( decoder_log ) fprintf(decoder_log, " BAD\n");
			continue;
		}
		...
		if( decoder_log ) fprintf(decoder_log, " GOOD ");
		if( decoder_log ) fprintf(decoder_log, ": %14llu %4llu %14llu %5hu %20llu %15.12g", rx_timetag, rx_timetag_wraps, rx_timetag2, t1coarse, absoluteT1, absoluteT1 / getFrequency() );
		if( decoder_log ) fprintf(decoder_log, ": %2hu %2hu %1hu : %6hu %4hu %4hu; %4hu %4hu %4hu", e.channelID / 16, e.channelID % 16, e.tacID, t1coarse % 1024, e.t2coarse, e.qcoarse, e.t1fine, e.t2fine, e.qfine);
		
		if( decoder_log ) fprintf(decoder_log, "\n");
		//if( decoder_log ) fprintf(decoder_log, ": %2hu %2hu %1hu ; %6hu %4hu %4hu; %4hu %4hu %4hu\n", elink, e.channelID % 16, e.tacID, t1coarse % 1024, e.t2coarse, e.qcoarse, e.t1fine, e.t2fine, e.qfine);
		e.valid = true;
